...
export interface ArticleFilters {
  ...
  startDateFrom?: Date;
  startDateTo?: Date;
  /** ✅ filtre les articles/vidéos liés à une destination donnée */
...
export class ArticleService {
  ...
  async getArticles(
    ...
    // ✅ Filtre par destination. Le scalaire `destinationId` est bien une
    // FK explicite dans le schéma (Article.destinationId Int? avec
    // @relation), donc le filtre direct est syntaxiquement correct.
    // On restreint aussi explicitement aux types ARTICLE/VIDEO pour ne
    // jamais inclure une fiche PAGE/SALON/DESTINATION mal associée.
    ...
    // Affiche dans les logs serveur le `where` réellement envoyé à Prisma
    // pour cette requête. Si tu vois `destinationId: undefined` ou
    // l'absence totale de la clé alors que tu as bien cliqué sur une
    // destination, le problème est dans le controller (filters.destinationId
    // n'arrive jamais jusqu'ici). Si tu vois la bonne valeur ici mais que
    // le résultat contient quand même des articles d'autres destinations,
    // le problème est en base (données) ou dans le cache HTTP, pas dans
    // ce service.
    console.log('[ArticleService.getArticles] filters reçus =', filters);
    console.log('[ArticleService.getArticles] where Prisma =', JSON.stringify(where));
    ...
    // 🔍 DEBUG TEMPORAIRE — vérifie les destinationId réellement renvoyés
    console.log(
      '[ArticleService.getArticles] résultats →',
      articles.map((a) => ({ id: a.id, title: a.title, destinationId: a.destinationId }))
    );
