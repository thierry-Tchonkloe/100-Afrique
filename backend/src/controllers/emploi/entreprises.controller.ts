...
// // FIX (système de secteur / localisation) : Etablissement.sector et
// // Etablissement.city ne sont pas toujours renseignés (comptes créés avant
// // la synchronisation vitrine → établissement, ou synchro qui a échoué une
// // fois). On retombe alors sur les valeurs saisies côté Vitrine, qui sont
// // désormais la source la plus fiable après une sauvegarde dashboard.
// function fmtCompany(e: any) {
//   const vitrine = e.vitrine;
...
//     sector:      e.sector || vitrine?.sector   || '',
//     city:        e.city   || vitrine?.location || '',
//     // Logo : priorité au logo Etablissement, sinon celui de la Vitrine
//     logo:        e.logo ?? vitrine?.logoUrl ?? null,
//     offresCount: e.offres.length,
//     vitrine: vitrine
//       ? {
...
// // indépendante des offres. Une entreprise apparaît dès qu'elle a une vitrine
// // complétée ou au moins une offre active (évite d'afficher des comptes tests vides).
// export async function getPublicCompanies(_req: Request, res: Response): Promise<void> {
//   try {
//     const etablissements = await prisma.etablissement.findMany({
//       include: {
...
// // ── GET /api/emploi/entreprises/:id ───────────────────────────────────────────
// // Détail public d'une entreprise (page vitrine publique / entreprises/[id])
// export async function getPublicCompanyDetail(req: Request, res: Response): Promise<void> {
//   try {
//     const id = Number(req.params.id);
//     const e = await prisma.etablissement.findUnique({
...
//           completionScore: vitrine.completionScore ?? 0,
//           views:           vitrine.views           ?? 0,
//           // FIX TS6133 : appel direct de firstPhotoUrl(vitrine.photos),
//           // plus de variable 'photos' intermédiaire jamais lue.
//           galleryImage:    firstPhotoUrl(vitrine.photos),
//         }
//       : null,
//   };
...
// // indépendante des offres. Une entreprise apparaît dès qu'elle a une vitrine
// // complétée ou au moins une offre active (évite d'afficher des comptes tests vides).
// // FIX TS6133 : req renommé en _req, non utilisé dans cette fonction.
// export async function getPublicCompanies(_req: Request, res: Response): Promise<void> {
//   try {
//     const etablissements = await prisma.etablissement.findMany({
//       include: {
...
//           salaryMin: o.salaryMin, salaryMax: o.salaryMax, remote: o.remote,
//           isPremium: o.isPremium,
//           // FIX ts18047 : publishedAt est `DateTime?` en Prisma (nullable),
//           // donc potentiellement null même avec le filtre status:'ACTIVE'.
//           // On retombe sur createdAt-like fallback via une chaîne vide plutôt
//           // que planter — en pratique une offre ACTIVE a presque toujours
//           // publishedAt, mais TS ne peut pas le garantir statiquement.
//           publishedAt: o.publishedAt ? o.publishedAt.toISOString() : null,
//         })),
//       },
//     });
