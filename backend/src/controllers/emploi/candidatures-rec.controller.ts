...
// const STATUS_MAP: Record<string, any> = {
//   new: 'SENT', sent: 'SENT', viewed: 'VIEWED',
//   in_progress: 'IN_PROGRESS', selected: 'SELECTED',
//   interview: 'INTERVIEW', accepted: 'ACCEPTED',
//   refused: 'REFUSED', archived: 'ARCHIVED',
// };
// const STATUS_REV: Record<string, string> = {
//   SENT: 'sent', VIEWED: 'viewed', IN_PROGRESS: 'in_progress',
//   SELECTED: 'selected', INTERVIEW: 'interview', ACCEPTED: 'accepted',
//   REFUSED: 'refused', ARCHIVED: 'archived', PENDING: 'new',
// };

// function calcMatchScore(app: any): number {
//   // Simple heuristic: based on experience count + skills overlap
//   const expCount = app.user.candidatProfil?.experiences?.length ?? 0;
//   return Math.min(50 + expCount * 10 + Math.floor(Math.random() * 20), 99);
// }
...
//     offerTitle: a.offre.title,
//     receivedAt: a.appliedAt.toISOString(),
//     status: STATUS_REV[a.status] ?? 'sent',
//     isRead: a.isRead,
//     isFavorite: a.isFavorite,
//     cvUrl: profil?.cvFileUrl ?? undefined,
//     experiences: (profil?.experiences ?? []).map((e: any) => ({
//       jobTitle: e.jobTitle, company: e.companyName,
//       period: `${e.startDate}${e.endDate ? ` - ${e.endDate}` : ' - En poste'}`,
//       description: (e.missions as string[])[0] ?? '',
//     })),
//     formations: (profil?.formations ?? []).map((f: any) => ({
//       diploma: f.diploma, school: f.school, year: f.year,
//     })),
//     skills: (profil?.hardSkills as string[]) ?? [],
//     location: profil?.city ?? '',
//     mobility: profil?.mobility ?? '',
...
//     const stats = {
//       new:         apps.filter((a) => a.status === 'SENT' && !a.isRead).length,
//       in_progress: apps.filter((a) => ['IN_PROGRESS', 'SELECTED', 'VIEWED'].includes(a.status)).length,
//       interview:   apps.filter((a) => a.status === 'INTERVIEW').length,
//       favorite:    apps.filter((a) => a.isFavorite).length,
...
//     const app = await prisma.application.findUnique({ where: { id: Number(req.params.id) } });
//     if (!app) { res.status(404).json({ success: false, message: 'Candidature introuvable' }); return; }
...
//     // Notify candidat
//     const notifType = status === 'refused' ? 'APPLICATION_REFUSED'
//       : status === 'accepted' || status === 'interview' ? 'APPLICATION_ACCEPTED'
//       : null;
//     if (notifType) {
//       const offre = await prisma.offre.findUnique({ where: { id: app.offreId }, select: { title: true, etablissement: { select: { name: true } } } });
//       await prisma.emploiNotification.create({
//         data: {
//           userId: app.userId,
//           type: notifType as any,
...
//     res.json({ success: true, data: { status: STATUS_REV[updated.status] } });
...
//     const app = await prisma.application.update({
//       where: { id: Number(req.params.id) },
//       data: { isRead: true, status: 'VIEWED' },
//     });

//     // Notify candidat that profile was viewed
//     await prisma.emploiNotification.create({
//       data: {
//         userId: app.userId,
//         type: 'PROFILE_VIEWED',
...
//   try {
//     const { isFavorite } = req.body;
//     await prisma.application.update({ where: { id: Number(req.params.id) }, data: { isFavorite } });
//     res.json({ success: true });
//   } catch { res.status(500).json({ success: false, message: 'Erreur serveur' }); }
// }
...
//     // Here: create a notification for the candidat
//     await prisma.emploiNotification.create({
//       data: {
//         userId: app.userId,
//         type: 'APPLICATION_ACCEPTED',
...
//   try {
//     const { starred } = req.body;
//     await prisma.application.update({ where: { id: Number(req.params.id) }, data: { isFavorite: starred } });
//     res.json({ success: true });
//   } catch { res.status(500).json({ success: false, message: 'Erreur serveur' }); }
// }