...
//
// FIX : ajout du champ "Nom de l'entreprise" dans le formulaire d'inscription,
// affiché uniquement pour le rôle RECRUITER. Sans ce champ, aucun nom fiable
// n'est rattaché à l'Établissement créé côté backend, et l'entreprise ne peut
// pas être affichée correctement dans la section COMPANIES de la page d'accueil.
...
function RegisterForm({ onSuccess }: { onSuccess: (role: string) => void }) {
  ...
  // ── NOUVEAU : nom de l'entreprise, requis uniquement pour un compte recruteur.
  // Ce nom est ce qui sera rattaché à l'Établissement créé côté backend, et donc
  // ce qui sera affiché dans la section COMPANIES de la page d'accueil Emploi.
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // ── NOUVEAU : validation du nom d'entreprise, uniquement pour RECRUITER
    ...
    try {
      const res = await registerEmploi({
        ...
        lastName,
        role,
        // ── NOUVEAU : n'envoyer companyName que pour un recruteur
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {/* ── NOUVEAU : Nom de l'entreprise — visible uniquement si RECRUITER ── */}
      {role === 'RECRUITER' && (
        <Input
