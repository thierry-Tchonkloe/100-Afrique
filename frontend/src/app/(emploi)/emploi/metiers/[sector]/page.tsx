...
// ── Auth helpers (même pattern que EmploiHeader / ApplyButton) ────────────────
...
// ── Alert CTA Button — auth-aware, même logique que ApplyButton ──────────────
...
function SectorContent() {
  ...
  // Si le segment d'URL est déjà une clé canonique connue, on l'utilise telle quelle.
  // Sinon (ex: quelqu'un tape /emploi/metiers/Hôtellerie à la main), on tente de la
  // normaliser. Si ça échoue vraiment, `cfg` sera undefined et on redirigera.
  const sector = SECTORS[rawSector] ? rawSector : (normalizeSector(rawSector) || rawSector);
  ...
  useEffect(() => {
    if (rawSector && !SECTORS[sector]) {
      router.replace('/emploi/jobs');
    }
  }, [rawSector, sector, router]);
...
//   // Candidat connecté → accès direct à son dashboard d'alertes
//   if (user && getAuthToken() && user.role === 'CANDIDAT') {
//     return (
//       <button
//         onClick={() => router.push('/candidat/alertes')}
...
//   // Recruteur connecté → non éligible aux alertes candidat
//   if (user && getAuthToken() && user.role === 'RECRUITER') {
//     return (
//       <div
//         className="flex items-center justify-center gap-1.5 w-full bg-white/60 text-sm font-bold
...
//   // Non connecté → auth avec redirect vers le dashboard d'alertes
//   return (
//     <button
//       onClick={() => router.push('/auth?redirect=/candidat/alertes')}
//       className="flex items-center justify-center gap-1.5 w-full bg-white text-sm font-bold
