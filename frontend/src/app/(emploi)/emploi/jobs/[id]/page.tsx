...
// le libellé brut stocké en base. On passe toujours par normalizeSector() avant lookup.
...
export default function JobDetailPage() {
  ...
  // ── Normalisation du secteur : job.sector est un libellé FR brut venant de la DB
  // (ex: "Hôtellerie", "Spa & Bien-être"...). sectorKey sert aux icônes/couleurs,
  // browseUrl gère le repli si aucune page métier n'existe pour ce secteur.
          ...
          <div className="space-y-4">
            ...
            {similar.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                ...
                {/* ── FIX : navigation via router.push + URL toujours valide.
                    browseUrl pointe vers /emploi/metiers/{clé} quand la page métier
                    existe, sinon vers /emploi/jobs?sector=... (repli sûr qui ne
                    redirige jamais automatiquement). */}
                <button
                  type="button"
...
// // Libellés affichés pour le lien "Voir toutes les offres · {label}"
...
//   // Avant hydration : bouton neutre pour éviter le flash
//   if (!hydrated) {
//     return (
//       <button
//         disabled
...
//     ? `${Math.round(job.salaryMin / 1000)}–${Math.round((job.salaryMax ?? job.salaryMin) / 1000)}k€ / an`
//     : null;
//   const sectorLabel = SECTOR_LABEL[job.sector] ?? job.sector;
...
//                 {/* ── FIX : navigation forcée via router.push (comme MetierCard) ──
//                     Un <Link> statique ici pouvait ne pas déclencher de vraie
//                     navigation dans certains cas (state du Link resté "stale").
//                     On utilise le même pattern que MetierCard / SimilarJobCard
//                     pour garantir la navigation vers /emploi/metiers/[sector]. */}
//                 <button
//                   type="button"
//                   onClick={() => router.push(`/emploi/metiers/${job.sector}`)}
//                   className="flex items-center justify-center gap-1.5 w-full mt-3 pt-3 border-t border-gray-50
//                              text-xs font-semibold text-[#E8622A] hover:underline"
//                 >
//                   Voir toutes les offres · {sectorLabel} <ChevronRight size={12} />
//                 </button>
//               </div>
//             )}
