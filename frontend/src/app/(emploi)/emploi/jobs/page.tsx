...
import {
  ...
  Banknote, Car, Star, Send, LogIn, Lock, Loader2, AlertCircle,
...
// ── Debounce hook ─────────────────────────────────────────────────────────────
// FIX (moteur de recherche) : évite un appel réseau à chaque frappe clavier
// dans les champs "Recherche" / "Localisation".
...
// FIX (moteur de recherche) : valeurs alignées sur le schéma réel de l'offre
// (remote: 'none' | 'partial' | 'full'), au lieu des libellés FR qui n'étaient
...
// ── Apply Button inline ───────────────────────────────────────────────────────
// Même logique exacte que dans jobs/[id]/page.tsx — isolé en composant réutilisable.
...
function InlineApplyButton({ jobId }: { jobId: string }) {
  ...
  // Avant hydration : bouton neutre
  if (!hydrated) {
    return (
      <button disabled
        className="flex items-center gap-1.5 text-xs font-semibold bg-[#E8622A]/60
  ...
  if (!user || !getAuthToken()) {
    return (
      <button
        ...
        className="flex items-center gap-1.5 text-xs font-semibold bg-[#E8622A]
                   hover:bg-[#d4561f] text-white px-3 py-1.5 rounded-lg transition-colors"
  ...
  if (user.role === 'RECRUITER') {
    return (
      <button
        disabled
        className="flex items-center gap-1.5 text-xs font-semibold bg-gray-100
                   text-gray-400 px-3 py-1.5 rounded-lg cursor-not-allowed"
  ...
  if (applied) {
    return (
      <span className="flex items-center gap-1.5 text-xs font-semibold
                       bg-green-50 text-green-600 border border-green-100 px-3 py-1.5 rounded-lg">
  ...
  if (applyErr) {
    return (
      <span className="flex items-center gap-1.5 text-xs font-semibold
                       bg-amber-50 text-amber-600 border border-amber-100 px-3 py-1.5 rounded-lg">
        <AlertCircle size={12} /> Déjà postulé
      </span>
    );
  }

  // Candidat connecté — postuler
  async function handleApply(e: React.MouseEvent) {
    ...
    try {
      ...
      if (status === 409) {
        ...
        setApplied(true); // fallback optimiste comme dans le dashboard
      }
    } finally {
      setApplying(false);
    }
  ...
  return (
    <button
      ...
      disabled={applying}
      className="flex items-center gap-1.5 text-xs font-semibold bg-[#E8622A]
                 hover:bg-[#d4561f] disabled:opacity-60 text-white px-3 py-1.5
    ...
    <div className="border-b border-gray-100 last:border-0">
      <button
        ...
        className="w-full flex items-center justify-between py-3 text-sm font-semibold
                   text-gray-700 hover:text-[#E8622A] transition-colors"
      >
        {title}
...
// ── Job Card ──────────────────────────────────────────────────────────────────
// Cliquable pour aller au détail + bouton Postuler inline.
    ...
    <div className="bg-white rounded-2xl border border-gray-100 hover:border-gray-200
      ...
      <div className="p-5">
        <div className="flex gap-4">
          ...
          {/* Icône secteur — cliquable vers détail */}
          <button
            onClick={() => router.push(`/emploi/jobs/${offre.id}`)}
            className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0
                        ${iconColor} hover:opacity-80 transition-opacity`}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                ...
                <button
                  ...
                  className="font-bold text-[#1E2A3A] text-base group-hover:text-[#E8622A]
                             transition-colors leading-tight text-left hover:underline line-clamp-1 w-full"
                >
                  {offre.title}
                </button>
                <p className="text-[#E8622A] text-sm font-medium mt-0.5 truncate">
                  {offre.companyName}
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); onFavorite(offre.id); }}
      ...
      <div className="px-5 pb-4 flex items-center justify-between gap-3 border-t border-gray-50 pt-3">
        ...
        {/* Bouton postuler inline avec gestion auth */}
...
function Sidebar({ filters, setFilters, total, onClose }: {
  ...
  return (
    <aside className="w-full bg-white rounded-2xl border border-gray-100 p-4">
      ...
      {/* FIX (moteur de recherche) : options alignées sur le schéma réel
          (full | partial | none), toutes cochables (multi-sélection) et
          réellement transmises au backend via filters.remote.join(','). */}
      <FilterSection title="Télétravail">
        {REMOTE_OPTIONS.map(({ value, label }) => (
          <CheckItem key={value} label={label}
            checked={filters.remote.includes(value)}
            onChange={() => toggle('remote', value)} />
        ))}
      </FilterSection>
...
function JobsContent() {
  ...
  // FIX (moteur de recherche) : debounce sur les champs texte pour éviter un
  ...
  const loadJobs = useCallback(async (reset = false) => {
    ...
    try {
      const res = await fetchPublicJobs({
        ...
        // FIX : envoie toutes les valeurs sélectionnées, pas seulement la 1ère
  ...
  useEffect(() => {
    setPage(1);
    loadJobs(true);
  }, [debouncedSearch, debouncedLocation, filters.contractTypes, filters.remote]); // eslint-disable-line
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="lg:hidden flex items-center gap-2 border border-gray-200 bg-white
                             rounded-xl px-3 py-2 text-sm font-medium text-gray-600
                             hover:border-[#E8622A] transition"
                >
              ...
              <div className="relative">
                ...
                <ArrowUpDown size={13} className="absolute right-2.5 top-1/2 -translate-y-1/2
            ...
            {activeFilters > 0 && (
              <div className="flex flex-wrap gap-2 mb-4">
                {[
                  ...
                  <span key={f}
                    className="flex items-center gap-1 bg-white border border-[#E8622A]/30
                    ...
                    <button
                      onClick={() => setFilters((prev) => ({
                        ...
                        remote:        prev.remote.filter(
                          (x) => (REMOTE_OPTIONS.find((o) => o.value === x)?.label ?? x) !== f,
                        ),
              ...
              <div className="space-y-3">
                ...
                {hasMore && (
                  <div className="flex justify-center pt-4">
                    <button
                      onClick={() => { setPage((p) => p + 1); loadJobs(false); }}
