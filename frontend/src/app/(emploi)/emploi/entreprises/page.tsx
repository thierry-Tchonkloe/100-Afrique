...
//
// FIX (localisation + secteur) : la localisation ne se limite plus à une
// icône sans texte — un libellé "Localisation non renseignée" s'affiche si
// company.city est vide. Le badge secteur ne s'affiche que si sector existe
// (évite un badge vide illisible pour les entreprises non encore synchronisées).
...
import {
  ...
  Briefcase, Users, Star, Heart, ArrowRight,
  LayoutGrid, List, Hotel, Utensils, Plane, Monitor,
  Calendar, Building2, RotateCcw, Check, Zap,
} from 'lucide-react';
import {
...
import { SECTOR_DEFS, SECTOR_LABELS, normalizeSector, type SectorKey } from '@/lib/sectors';
...
const SECTOR_ICONS: Record<SectorKey, React.ReactNode> = {
  ...
  spa:           <Star     size={16} />,
  entertainment: <Building2 size={16} />,
};

const SECTOR_COLORS: Record<SectorKey, { bg: string; text: string; light: string }> = {
...
const FEATURED_SECTORS = SECTOR_DEFS.map((s) => ({
  key:   s.key,
  label: s.label,
...
function getLogoImage(company: Company): string | undefined {
    ...
    <div className="group bg-white rounded-2xl border border-gray-100 overflow-hidden
      ...
      <div className="pt-8 pb-5 px-4 flex-1 flex flex-col">
        <div>
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              {/* FIX : badge secteur affiché uniquement s'il existe réellement */}
              {company.sector && (
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-50 mt-4">
          {/* FIX : localisation affichée avec fallback texte au lieu d'une icône seule */}
          ...
          {SECTOR_DEFS.map((s) => (
            <label key={s.key} className="flex items-center gap-2.5 cursor-pointer py-0.5 group">
              <div
                ...
                className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition
                  ${filters.sectors.includes(s.key)
                    ? 'bg-[#E8622A] border-[#E8622A]'
                    : 'border-gray-300 group-hover:border-[#E8622A]'}`}
              >
...
// // src/app/(emploi)/emploi/entreprises/page.tsx
// //
// // FIX (système de secteur) : SECTORS, FEATURED_SECTORS, SECTOR_ICONS et
// // SECTOR_COLORS dérivent maintenant de SECTOR_DEFS (src/lib/sectors.ts).
// // Le filtre par carousel/sidebar utilise des CLÉS canoniques (activeSector,
// // filters.sectors), comparées à company.sector (libellé brut) via
// // normalizeSector() plutôt que des sous-chaînes approximatives.
...
// // FIX : plus de libellés inventés ("Aérien & Transport", "Croisières"...) —
// // utilise directement la liste canonique des 8 secteurs réels.
// const COMPANY_SIZES = [
//   'Toutes tailles',
//   'Startup (< 50)',
//   'PME (50–250)',
...
// // Enrichissements mock — repli visuel/texte uniquement, quand l'entreprise
// // réelle n'a pas encore complété sa vitrine.
// const COMPANY_ENRICHMENTS: Record<string, Partial<Company>> = {
//   'Luxury Hotels Group': {
//     description: 'Chaîne hôtelière de prestige avec 45 établissements en Europe. Excellence, innovation et art de vivre à la française.',
//     tags: ['Great Place to Work', 'Éco-responsable'],
...
// // FIX : dérivé de SECTOR_DEFS — même clé, même libellé que partout ailleurs.
// const FEATURED_SECTORS = SECTOR_DEFS.map((s) => ({
//   key:   s.key,
//   label: s.label,
//   icon:  SECTOR_ICONS[s.key],
...
// /** Bannière réelle : vitrine en priorité, puis galerie, puis mock */
// function getCoverImage(company: Company): string | undefined {
//   return company.vitrine?.bannerUrl
//     ?? company.vitrine?.galleryImage
//     ?? COMPANY_ENRICHMENTS[company.name]?.coverUrl
...
// /** Logo réel : Etablissement.logo en priorité, sinon Vitrine.logoUrl */
// function getLogoImage(company: Company): string | undefined {
//   return company.logo ?? company.vitrine?.logoUrl ?? undefined;
// }
...
// interface FilterState {
//   sectors: string[]; // clés canoniques (SectorKey), pas des libellés
//   sizes: string[];
//   hasOffres: boolean;
//   isPremium: boolean;
//   isRecruiting: boolean;
...
//         {/* FIX : filtre secteur basé sur les CLÉS canoniques (SECTOR_DEFS),
//             plus jamais une liste de libellés inventés localement. */}
//         <div className="space-y-2">
//           <p className="text-xs font-bold text-gray-700 uppercase tracking-wider">Secteur</p>
//           {SECTOR_DEFS.map((s) => (
//             <label key={s.key} className="flex items-center gap-2.5 cursor-pointer py-0.5 group">
...
//   const [search,         setSearch]         = useState(searchParams.get('search') ?? '');
//   const [location,       setLocation]       = useState(searchParams.get('location') ?? '');
//   // FIX : activeSector est une CLÉ canonique (ex: "hotel"), plus un libellé
//   const [activeSector,   setActiveSector]   = useState(searchParams.get('sector') ?? '');
//   const [sort,           setSort]           = useState<SortKey>('pertinence');
//   const [viewMode,       setViewMode]       = useState<ViewMode>('grid');
//   const [filters,        setFilters]        = useState<FilterState>(EMPTY_FILTERS);
...
//     // FIX : comparaison via normalizeSector() — company.sector est un libellé
//     // brut ("Hôtellerie"), activeSector est une clé ("hotel").
//     if (activeSector) {
//       list = list.filter((c) => normalizeSector(c.sector) === activeSector);
//     }
...
//           {/* Sector pill carousel — utilise les clés canoniques */}
//           <div className="flex gap-2.5 mt-4 overflow-x-auto pb-1 scrollbar-hide">
//             <button
//               onClick={() => setActiveSector('')}
//               className={`flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-full border
...
// // src/app/(emploi)/emploi/entreprises/page.tsx
// //
// // FIX (images entreprises) : le chargement des entreprises utilise maintenant
// // fetchPublicCompanies() (basé sur Etablissement + Vitrine) au lieu de dériver
// // la liste depuis les offres publiques, qui ne contenaient aucune image.
// // Les cards (grid + list) affichent désormais en priorité les vraies images
// // de la vitrine (logo, bannière, première photo de galerie) avant de retomber
// // sur les enrichissements mock.
...
// // Enrichissements mock — utilisés uniquement comme repli visuel/texte quand
// // l'entreprise réelle n'a pas encore complété sa vitrine.
// const COMPANY_ENRICHMENTS: Record<string, Partial<Company>> = {
//   'Luxury Hotels Group': {
//     description: 'Chaîne hôtelière de prestige avec 45 établissements en Europe. Excellence, innovation et art de vivre à la française.',
//     tags: ['Great Place to Work', 'Éco-responsable'],
...
// /**
//  * Image de couverture réelle d'une entreprise : bannière de vitrine en priorité,
//  * puis première photo de galerie, puis repli sur le mock d'enrichissement.
//  */
// function getCoverImage(company: Company): string | undefined {
//   return company.vitrine?.bannerUrl
//     ?? company.vitrine?.galleryImage
...
// /**
//  * Logo réel d'une entreprise : Etablissement.logo en priorité, sinon Vitrine.logoUrl.
//  */
// function getLogoImage(company: Company): string | undefined {
//   return company.logo ?? company.vitrine?.logoUrl ?? undefined;
// }
...
//   // ── FIX : images réelles de la vitrine en priorité sur les mocks ──────────
//   const coverImage = getCoverImage(company);
//   const logoImage  = getLogoImage(company);
...
//         {/* Logo — FIX : image réelle si disponible, sinon icône + couleur mock */}
//         <div className={`absolute bottom-0 left-4 translate-y-1/2 w-12 h-12 rounded-xl shadow-lg
//                          flex items-center justify-center border-2 border-white overflow-hidden z-10
//                          ${logoImage ? 'bg-white' : (enriched?.logoColor ?? 'bg-[#1E2A3A]')}`}>
//           {logoImage ? (
...
//   // ── FIX : image réelle du logo en priorité sur le mock ────────────────────
//   const logoImage = getLogoImage(company);
...
//         <div className="flex items-center gap-4 p-4 flex-1 min-w-0">
//           {/* Logo — FIX : image réelle si disponible */}
//           <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 border border-gray-100
//                            shadow-sm overflow-hidden ${logoImage ? 'bg-white' : (enriched?.logoColor ?? 'bg-[#1E2A3A]')}`}>
//             {logoImage ? (
//               <img src={logoImage} alt={company.name} className="w-full h-full object-contain p-1" />
...
//   // ── FIX : chargement via le vrai endpoint /entreprises (Etablissement + Vitrine)
//   // au lieu de grouper les offres publiques, qui ne contiennent aucune image.
//   useEffect(() => {
//     (async () => {
//       setLoading(true);
//       try {
