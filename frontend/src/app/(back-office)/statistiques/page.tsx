// src/app/(back-office)/statistiques/page.tsx
"use client";

import React, { useState } from "react";
import {
  LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer,
  PieChart, Pie, Cell, BarChart, Bar, AreaChart, Area, CartesianGrid,
} from "recharts";
import {
  Users, FileText, Clock, TrendingUp, TrendingDown, ExternalLink,
  Download, ChevronDown, Play, MousePointer, Mail, Eye,
  RefreshCw, AlertCircle, Loader2,
} from "lucide-react";
import { ProtectedRoute } from "@/components/Dashboard/ProtectedRoute";
import { useStatsPageData } from "@/hooks/useDashboardStats";
...
function SkeletonBlock({ h = "h-8", w = "w-28" }: { h?: string; w?: string }) {
  return <div className={`${h} ${w} bg-gray-100 animate-pulse rounded`} />;
}

const StatCard = ({
  icon, label, value, delta, positive, sub, loading,
}: {
...
const ContentRow = ({
  title, subtitle, vues, highlight,
}: {
  title: string; subtitle: string; vues: string; highlight?: boolean;
}) => (
  <div className="flex items-center justify-between py-2.5 border-b border-gray-50 last:border-0">
    ...
    <span className={`ml-3 text-sm font-bold shrink-0 ${highlight ? "text-amber-500" : "text-gray-600"}`}>
    ...
    <div className="space-y-3">
      {Array.from({ length: n }).map((_, i) => (
        <div key={i} className="flex items-center justify-between py-2.5 border-b border-gray-50">
          ...
          <div className="h-4 bg-gray-100 animate-pulse rounded w-16 ml-4 shrink-0" />
        </div>
      ))}
    </div>
  );
...
export default function StatsDashboard() {
  ...
  const { data, loading, error, reload } = useStatsPageData();
      ...
      <div className="min-h-screen bg-gray-50 font-sans">
        <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                onClick={reload}
                disabled={loading}
                className="flex items-center gap-2 text-xs text-gray-500 border border-gray-200 rounded-lg px-3 py-2 hover:bg-gray-50 transition disabled:opacity-40"
              >
          ...
          <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            {/* Line Chart */}
            <div className="mb-6">
              <p className="text-xs text-gray-400 text-center mb-2 tracking-wide">
                Évolution du Trafic
              </p>
              {loading ? (
              ...
              <div className="flex justify-center gap-6 mt-2">
                {[
                  { color: "#f59e0b", label: "Visiteurs Uniques" },
                  { color: "#1e3a8a", label: "Pages Vues" },
                ].map((l) => (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
              <StatCard
                ...
                delta={data?.kpis.uniqueVisitors.delta ?? ""}
                positive={data?.kpis.uniqueVisitors.positive ?? true}
              ...
              <StatCard
                ...
                positive={data?.kpis.pageViews.positive ?? true}
                sub="période précédente"
                loading={loading}
              />
              <StatCard
                ...
                value={data?.kpis.bounceRate.value ?? "–"}
                delta={data?.kpis.bounceRate.delta ?? ""}
                positive={data?.kpis.bounceRate.positive ?? false}
                sub="période précédente"
                loading={loading}
              />
              <StatCard
                ...
                delta={data?.kpis.avgDuration.delta ?? ""}
                positive={data?.kpis.avgDuration.positive ?? true}
                sub="période précédente"
                loading={loading}
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                {loading ? (
                  ...
                  <p className="text-sm text-gray-400 text-center py-4">Aucun article trouvé.</p>
                ) : (
              ...
              <div>
                ...
                {loading ? (
                  ...
                  <p className="text-sm text-gray-400 text-center py-4">Aucune destination trouvée.</p>
                ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                ...
                {loading ? (
                  ...
                  <ResponsiveContainer width="100%" height={160}>
                    <BarChart data={data?.viewingTimeData ?? []} barCategoryGap="30%">
                      <XAxis dataKey="label" tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} />
                      <YAxis hide />
                      <Tooltip formatter={(v: number | undefined) => `${v ?? 0}h`} />
              ...
              <div>
                ...
                {loading ? (
                  ...
                  <p className="text-sm text-gray-400 text-center py-4">Aucune vidéo trouvée.</p>
                ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              ...
              <div className="rounded-xl bg-gray-50 p-4 border border-gray-100">
                ...
                <div className="space-y-1 mt-2">
                  {loading ? (
                    [0, 1, 2].map((i) => (
                      <div key={i} className="h-3.5 bg-gray-200 animate-pulse rounded w-full" />
                    ))
                  ) : (
                    [
                      { label: "Droit Ancrage", value: "–" },
                      { label: "XX Hôtels", value: "–" },
                      { label: "Forfait Fin de vols", value: "–" },
                    ].map((row) => (
                  ...
                  {loading ? (
                    ...
                    <>
                      <p className="text-2xl font-extrabold text-gray-900">
                        {data?.newsletterNew.toLocaleString("fr-FR") ?? "–"}
                      </p>
                      <p className="text-xs text-gray-400">nouveaux ce mois</p>
                    </>
                  )}
                </div>
                <p className="text-xs text-gray-400 mb-2">
                  Total : {loading ? "…" : (data?.newsletterTotal.toLocaleString("fr-FR") ?? "–")}
                </p>
                {loading ? (
                  ...
                  <ResponsiveContainer width="100%" height={70}>
                    <AreaChart data={data?.newsletterData ?? []}>
                      <defs>
