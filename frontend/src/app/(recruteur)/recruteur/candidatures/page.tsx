...
export default function CandidaturesRecuesPage() {
  ...
  function updateCandidature(id: string, patch: Partial<CandidatureRec>) {
    setData((prev: CandidaturesRecResponse) => {
      const updated = prev.candidatures.map((c) =>
        c.id === id ? { ...c, ...patch } : c,
      );

      // ── FIX: recompute stats cohérents avec le mapping backend
      const stats = {
        new: updated.filter((c) => c.status === 'new' && !c.isRead).length,
        in_progress: updated.filter(
          (c) => c.status === 'in_progress' || (c.status === 'new' && c.isRead),
        ).length,
  ...
  async function handleSelect(id: string) {
    ...
    if (cand && !cand.isRead) {
      // Marquer comme lu localement : passage de 'new' → 'in_progress'
  ...
  const filtered = useMemo(() => {
    return data.candidatures.filter((c) => {
      // ── FIX: filtre par onglet aligné sur le mapping de statuts
      let matchTab: boolean;
      if (activeTab === 'all') {
        matchTab = true;
      } else if (activeTab === 'favorite') {
        matchTab = c.isFavorite;
      } else if (activeTab === 'new') {
        // 'new' = status 'new' non lu OU status 'sent' non lu (compat. avant fix backend)
        matchTab = (c.status === 'new' || c.status === 'sent') && !c.isRead;
      } else if (activeTab === 'in_progress') {
        // 'in_progress' = status 'in_progress' OU 'new'/'sent' déjà lu
        matchTab =
          c.status === 'in_progress' ||
          ((c.status === 'new' || c.status === 'sent') && c.isRead);
      } else {
  ...
  // Le candidat sélectionné, ou le premier de la liste filtrée
  const selected =
    data.candidatures.find((c) => c.id === selectedId) ??
    filtered[0] ??
    null;
...
// import CandidateDetailPanel from '@/components/recruteur/candidatures/CandidateDetailPanel';
// import { markCandidatureRead } from '@/services/candidatures-rec.service';
// import type { CandidatureRec, CandidatureRecStatus, CandidaturesRecResponse } from '@/types/candidatures-rec.types';
// import clsx from 'clsx';
...
//   // ── Update candidature in local state (sans null check car data n'est plus null)
//   function updateCandidature(id: string, patch: Partial<CandidatureRec>) {
//     setData((prev: CandidaturesRecResponse) => {
//       const updated = prev.candidatures.map((c) =>
//         c.id === id ? { ...c, ...patch } : c
...
//       const stats = {
//         new:         updated.filter((c) => c.status === 'new'         && !c.isRead).length,
//         in_progress: updated.filter((c) => c.status === 'in_progress').length,
//         interview:   updated.filter((c) => c.status === 'interview'  ).length,
//         favorite:    updated.filter((c) => c.isFavorite               ).length,
//         refused:     updated.filter((c) => c.status === 'refused'     ).length,
//       };
...
//       const matchTab =
//         activeTab === 'all'      ? true :
//         activeTab === 'favorite' ? c.isFavorite :
//         c.status === activeTab;
