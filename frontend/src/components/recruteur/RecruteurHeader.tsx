...
// import Link from 'next/link';
// import { ChevronLeft, Plus, Menu, ChevronDown, User } from 'lucide-react';
// import type { RecruteurProfile, Etablissement } from '@/types/recruteur.types';
...
//   async function handleSwitch(etab: Etablissement) {
//     try { await switchEtablissement(etab.id); } catch {}
//     onEtablissementChange?.(etab.id);
...
//       {/* Company switcher */}
//       {profile && profile.etablissements.length > 0 && (
...
//           >
//             <span className="hidden sm:block">{activeEtab?.name ?? 'Établissement'}</span>
//             <ChevronDown size={14} className="text-gray-400" />
//           </button>

//           {showSwitcher && (
//             <div className="absolute right-0 top-full mt-1.5 bg-white rounded-2xl shadow-xl border
//                             border-gray-100 z-50 py-1 min-w-52 overflow-hidden">
//               {profile.etablissements.map((e) => (
//                 <button
//                   key={e.id}
//                   onClick={() => handleSwitch(e)}
//                   className={`w-full flex items-start gap-3 px-4 py-2.5 hover:bg-gray-50 transition text-left ${
//                     e.id === profile.activeEtablissementId ? 'bg-orange-50/50' : ''
//                   }`}
//                 >
//                   <div className="w-7 h-7 bg-[#1E2A3A] rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
//                     <span className="text-[#E8622A] text-xs font-bold">{e.name[0]}</span>
//                   </div>
//                   <div>
//                     <p className="text-xs font-semibold text-gray-800">{e.name}</p>
//                     <p className="text-[10px] text-gray-400">{e.sector}</p>
//                   </div>
//                 </button>
//               ))}
//             </div>
...
//           </p>
//         </div>
//         <div className="w-9 h-9 rounded-full bg-[#1E2A3A] flex items-center justify-center overflow-hidden border-2 border-gray-100">
//           {profile?.avatar ? (
//             <img src={profile.avatar} alt="" className="w-full h-full object-cover" />
//           ) : (
//             <span className="text-white text-xs font-bold">
