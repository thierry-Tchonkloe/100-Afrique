...
// // src/components/recruteur/offres/OffreModal.tsx
// 'use client';
...
//       <div className="flex flex-wrap gap-2 mb-2">
//         {tags.map((t) => (
//           <span key={t} className="flex items-center gap-1 bg-[#FFF3EC] text-[#E8622A] text-xs
//                                    font-semibold px-2.5 py-1 rounded-full border border-[#FDDCC8]">
//             {t}
...
// // ── Text area ─────────────────────────────────────────────────────────────────
// function RichTextarea({ value, onChange, placeholder, rows = 5 }: {
//   value: string; onChange: (v: string) => void; placeholder: string; rows?: number;
// }) {
//   return (
...
//                  focus:outline-none focus:ring-2 focus:ring-[#E8622A]/30 focus:border-[#E8622A]
...
//   function s1<K extends keyof OffreFormStep1>(k: K, v: OffreFormStep1[K]) {
//     setForm((f) => ({ ...f, step1: { ...f.step1, [k]: v } }));
//   }
...
//                   : i === step ? 'bg-[#E8622A] text-white'
//                   : 'bg-gray-100 text-gray-400'
//                 )}>
//                   {i < step ? <Check size={12} /> : i + 1}
//                 </div>
//                 <span className={`text-xs font-medium hidden sm:block ${i === step ? 'text-[#E8622A]' : i < step ? 'text-green-500' : 'text-gray-400'}`}>
//                   {s}
//                 </span>
...
//                 <input value={form.step1.title} onChange={(e) => s1('title', e.target.value)}
//                   placeholder="ex: Chef de Réception H/F" className="input-f" />
//               </div>
//               <div className="grid grid-cols-2 gap-3">
//                 <div>
//                   <label className="label">Secteur *</label>
//                   <select value={form.step1.sector} onChange={(e) => s1('sector', e.target.value)} className="input-f bg-white">
//                     <option value="">Choisir…</option>
//                     {SECTORS.map((s) => <option key={s}>{s}</option>)}
//                   </select>
//                 </div>
...
//                   <select value={form.step1.contractType}
//                     onChange={(e) => s1('contractType', e.target.value as ContractType)} className="input-f bg-white">
//                     {CONTRACT_TYPES.map((c) => <option key={c}>{c}</option>)}
//                   </select>
//                 </div>
//                 <div>
//                   <label className="label">Localisation *</label>
//                   <input value={form.step1.location} onChange={(e) => s1('location', e.target.value)}
//                     placeholder="ex: Paris 8ème" className="input-f" />
//                 </div>
//                 <div>
//                   <label className="label">Télétravail</label>
//                   <select value={form.step1.remote}
//                     onChange={(e) => s1('remote', e.target.value as RemotePolicy)} className="input-f bg-white">
//                     {(Object.entries(REMOTE_OPTIONS) as [RemotePolicy, string][]).map(([k, v]) => (
//                       <option key={k} value={k}>{v}</option>
//                     ))}
//                   </select>
//                 </div>
//                 <div>
//                   <label className="label">Salaire min (€/an)</label>
//                   <input type="number" value={form.step1.salaryMin ?? ''}
//                     onChange={(e) => s1('salaryMin', e.target.value ? Number(e.target.value) : undefined)}
//                     placeholder="28000" className="input-f" />
//                 </div>
//                 <div>
//                   <label className="label">Salaire max (€/an)</label>
//                   <input type="number" value={form.step1.salaryMax ?? ''}
//                     onChange={(e) => s1('salaryMax', e.target.value ? Number(e.target.value) : undefined)}
//                     placeholder="35000" className="input-f" />
//                 </div>
//               </div>
//             </>
//           )}
...
//                 <RichTextarea value={form.step2.missions}
//                   onChange={(v) => setForm((f) => ({ ...f, step2: { ...f.step2, missions: v } }))}
//                   placeholder="Décrivez les missions du poste…" />
//               </div>
//               <div>
//                 <label className="label">Profil recherché *</label>
//                 <RichTextarea value={form.step2.profile}
//                   onChange={(v) => setForm((f) => ({ ...f, step2: { ...f.step2, profile: v } }))}
//                   placeholder="Décrivez le profil idéal…" rows={4} />
//               </div>
//               <div>
//                 <label className="label">Avantages & conditions</label>
//                 <RichTextarea value={form.step2.advantages}
//                   onChange={(v) => setForm((f) => ({ ...f, step2: { ...f.step2, advantages: v } }))}
//                   placeholder="Avantages, mutuelle, tickets-restaurant…" rows={3} />
//               </div>
//             </>
//           )}
...
//                 <TagInput tags={form.step3.requiredSkills}
//                   onChange={(t) => setForm((f) => ({ ...f, step3: { ...f.step3, requiredSkills: t } }))}
//                   suggestions={REQUIRED_SKILLS_SUGGESTIONS}
//                   placeholder="Tapez ou choisissez une compétence…" />
//               </div>
//               <div>
...
//                 <TagInput tags={form.step3.languages}
//                   onChange={(t) => setForm((f) => ({ ...f, step3: { ...f.step3, languages: t } }))}
//                   placeholder="ex: Anglais courant, Espagnol…" />
//               </div>
//               <div>
//                 <label className="label">Logiciels / Outils</label>
//                 <TagInput tags={form.step3.softwares}
//                   onChange={(t) => setForm((f) => ({ ...f, step3: { ...f.step3, softwares: t } }))}
//                   placeholder="ex: PMS Opera, Excel…" />
//               </div>
//             </>
//           )}
...
//                 <p className="font-bold text-gray-900 text-base">{form.step1.title || '—'}</p>
//                 <div className="flex flex-wrap gap-2">
//                   {[form.step1.sector, form.step1.contractType, form.step1.location].filter(Boolean).map((t, i) => (
//                     <span key={i} className="text-xs bg-white border border-gray-200 text-gray-600 px-2 py-1 rounded-full">{t}</span>
//                   ))}
//                 </div>
//               </div>
//               {form.step2.missions && (
//                 <div>
//                   <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Missions</p>
//                   <p className="text-sm text-gray-700 whitespace-pre-wrap">{form.step2.missions.slice(0, 200)}…</p>
//                 </div>
//               )}
//               {form.step3.requiredSkills.length > 0 && (
//                 <div>
...
//                   <div className="flex flex-wrap gap-2">
//                     {form.step3.requiredSkills.map((s) => (
//                       <span key={s} className="text-xs bg-[#FFF3EC] text-[#E8622A] border border-[#FDDCC8] px-2.5 py-1 rounded-full font-semibold">{s}</span>
//                     ))}
//                   </div>
//                 </div>
//               )}
...
//             className="flex items-center gap-1.5 px-4 py-2 text-sm text-gray-500 hover:bg-gray-100 rounded-xl transition">
//             <ChevronLeft size={15} /> {step === 0 ? 'Annuler' : 'Retour'}
//           </button>
//           {step < STEPS.length - 1 ? (
...
//               <Check size={15} /> {saving ? 'Publication…' : 'Publier l\'offre'}
//             </button>
//           )}
//         </div>
//       </div>
...
//         .input-f { width: 100%; border: 1px solid #E5E7EB; border-radius: 0.75rem; padding: 0.625rem 0.75rem;
//                    font-size: 0.875rem; color: #1F2937; outline: none; transition: all 0.15s; }
//         .input-f:focus { border-color: #E8622A; box-shadow: 0 0 0 2px rgba(232,98,42,0.15); }
//       `}</style>
//     </div>
//   );
// }