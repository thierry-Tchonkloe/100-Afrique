    ...
    <section className="py-12 px-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-6 mb-12">
          ...
          {continents.map((c) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-[11px] font-bold tracking-wider transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                filter === c
                  ? 'bg-it-terracotta text-white shadow-lg'
        ...
        <div className="relative w-full lg:w-80">
          ...
          <input
            ...
            disabled={loading}
            className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-it-emerald/10 outline-none transition-all disabled:opacity-50"
          />
        </div>
      </div>
      ...
      {loading && destinations.length === 0 ? (
        <div className="text-center py-20 bg-slate-50 rounded-2xl">
          <Loader2 className="animate-spin mx-auto mb-4 text-it-terracotta" size={40} />
