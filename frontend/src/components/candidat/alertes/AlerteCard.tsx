// src/components/candidat/alertes/AlerteCard.tsx
'use client';
...
export default function AlerteCard({ alerte, onToggle, onEdit, onDelete }: AlerteCardProps) {
  const freqLabel = FREQUENCY_LABELS[alerte.frequency] ?? alerte.frequency;
  const freqIcon  = FREQUENCY_ICONS[alerte.frequency] ?? '';

  // ← Sécurisation : tous les tableaux peuvent être undefined si API mal unwrappée
  const keywords      = alerte.keywords      ?? [];
  const contractTypes = alerte.contractTypes ?? [];

  const tags = [
    ...keywords,
    alerte.location || null,
    ...contractTypes,
    `${freqIcon} ${freqLabel}`,
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5 hover:border-gray-200 transition">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          {/* Toggle ← fix design : left-0.5 + translate-x-0 */}
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              type="button"
              onClick={() => onToggle(alerte.id, !alerte.isActive)}
              className={`relative inline-flex items-center w-10 h-5 rounded-full transition-colors flex-shrink-0 cursor-pointer ${
            ...
            >
              <span className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-200 ${
                alerte.isActive ? 'translate-x-5' : 'translate-x-0'
              }`} />
            </button>
