// src/components/candidat/parametres/SecuritySection.tsx
'use client';
...
export default function SecuritySection({ email, twoFactorEnabled, onTwoFactorChange }: Props) {
  ...
  async function handleEmailSave() {
    if (!newEmail || !/\S+@\S+\.\S+/.test(newEmail)) {
      setEmailError('Email invalide'); return;
    }
    if (!emailPwd) {
      setEmailError('Mot de passe requis'); return;
    }
    setEmailError('');
  ...
  async function handlePasswordSave() {
    ...
    if (strength < 2) {
      setPwdError('Mot de passe trop faible (minimum : Faible).'); return;
    }
    setPwdError('');
    setPwdStatus('saving');
    try {
  ...
  async function handleTwoFactor(v: boolean) {
    onTwoFactorChange(v); // optimistic
    try {
      await toggleTwoFactor(v);
    } catch {
      onTwoFactorChange(!v); // revert
    }
  }
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="bg-[#E8622A] hover:bg-[#D45520] text-white text-xs font-semibold
                           px-4 py-2 rounded-xl transition disabled:opacity-50"
              >
                {emailStatus === 'saving' ? 'Enregistrement…' : 'Confirmer'}
              </button>
              <button
                onClick={() => { setEditingEmail(false); setNewEmail(email); setEmailError(''); }}
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="flex-1 border border-gray-200 rounded-xl px-3 py-2.5 text-sm
                         text-gray-600 bg-gray-50/50 cursor-default"
            />
            <button
        ...
        <div className="space-y-2.5">
          ...
          {newPwd.length > 0 && (
            <div className="space-y-1">
              <div className="flex gap-1">
                {([1, 2, 3, 4] as PasswordStrength[]).map((i) => (
                  <div
                    key={i}
          ...
          {pwdStatus === 'ok' && (
            <p className="flex items-center gap-1 text-xs text-green-600 font-medium">
              <CheckCircle size={12} /> Mot de passe mis à jour avec succès
            </p>
          )}
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {twoFactorEnabled
              ...
              : 'Désactivée — renforcez la sécurité de votre compte'}
          </p>
        </div>
