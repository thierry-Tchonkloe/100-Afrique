...
const ModaleAnnonceur = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-it-emerald-dark/80 p-4 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="relative w-full max-w-2xl max-h-[95vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl md:p-10 animate-in zoom-in-95 duration-300">
        ...
        <button
          onClick={onClose}
          className="absolute right-6 top-6 text-gray-400 hover:text-it-terracotta transition-colors"
        >
          <X size={24} />
        </button>
        ...
        {!isSuccess ? (
          <>
            <div className="mb-8 text-center">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="text-right text-xs leading-tight text-gray-500">
                  Atteignez plus de <span className="font-bold text-it-terracotta">50 000</span> <br />
                  <span className="font-bold text-it-terracotta">professionnels</span> du tourisme
                </div>
              </div>
            </div>
            ...
            <div className="mb-6 flex flex-col items-center gap-2">
              <a
                ...
                target="_blank"
                className="flex items-center gap-2 text-sm font-medium text-it-emerald hover:underline"
                rel="noreferrer"
              >
                <MessageCircle size={18} fill="#2A7F5F" className="text-white" />
                WhatsApp Business
              </a>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <select
                  required
                  name="interest"
                  className="w-full rounded-lg border border-gray-200 p-3 text-sm outline-none focus:border-it-terracotta focus:ring-1 focus:ring-it-terracotta transition-all"
                >
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  ...
                  <input required name="firstName" type="text" placeholder="Ex: Jean" className="w-full rounded-lg border border-gray-200 p-3 text-sm outline-none focus:border-it-terracotta focus:ring-1 focus:ring-it-terracotta" />
                </div>
                <div>
                  <label className="mb-1 block text-sm font-semibold text-gray-700">Nom *</label>
                  <input required name="lastName" type="text" placeholder="Ex: Dupont" className="w-full rounded-lg border border-gray-200 p-3 text-sm outline-none focus:border-it-terracotta focus:ring-1 focus:ring-it-terracotta" />
                </div>
              </div>

              <div>
                <label className="mb-1 block text-sm font-semibold text-gray-700">Société / Organisation *</label>
                <input required name="company" type="text" placeholder="Nom de votre structure" className="w-full rounded-lg border border-gray-200 p-3 text-sm outline-none focus:border-it-terracotta focus:ring-1 focus:ring-it-terracotta" />
              </div>

              <div>
                <label className="mb-1 block text-sm font-semibold text-gray-700">Email Professionnel *</label>
                <input required name="email" type="email" placeholder="votre.email@entreprise.com" className="w-full rounded-lg border border-gray-200 p-3 text-sm outline-none focus:border-it-terracotta focus:ring-1 focus:ring-it-terracotta" />
              </div>

              <div>
                <label className="mb-1 block text-sm font-semibold text-gray-700">Téléphone (pour un rappel rapide) *</label>
                <input required name="phone" type="tel" placeholder="+229 XX XX XX XX" className="w-full rounded-lg border border-gray-200 p-3 text-sm outline-none focus:border-it-terracotta focus:ring-1 focus:ring-it-terracotta" />
              </div>

              <div>
                <label className="mb-1 block text-sm font-semibold text-gray-700">Décrivez brièvement votre besoin (Optionnel)</label>
                <textarea name="message" rows={3} placeholder="Dites-nous en plus sur vos objectifs..." className="w-full rounded-lg border border-gray-200 p-3 text-sm outline-none focus:border-it-terracotta focus:ring-1 focus:ring-it-terracotta resize-none"></textarea>
              </div>

              <button
                disabled={loading}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-it-terracotta py-4 text-sm font-bold uppercase tracking-wider text-white transition-all hover:bg-it-terracotta-dark disabled:bg-gray-400 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
          ...
          <div className="py-12 text-center animate-in zoom-in-90 duration-500">
            <div className="flex justify-center mb-6">
              <CheckCircle2 size={80} className="text-it-emerald" strokeWidth={1.5} />
            </div>
            ...
            <button
              onClick={onClose}
              className="bg-it-emerald-dark text-white px-8 py-3 rounded-full font-bold hover:bg-it-terracotta transition-colors"
            >
              RETOUR AU SITE
            </button>
          </div>
        ...
        {!isSuccess && (
          <div className="mt-6 text-[10px] leading-relaxed text-gray-500 text-center">
            En soumettant ce formulaire, vous acceptez notre <span className="cursor-pointer text-it-terracotta hover:underline font-bold">Politique de Confidentialité</span>.
          </div>
        )}
      </div>
    </div>
