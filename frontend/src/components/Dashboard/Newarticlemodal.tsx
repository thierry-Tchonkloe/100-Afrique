...
function extractArray<T>(json: any, keys: string[]): T[] {
    for (const key of keys) {
        const value = key === "" ? json : key.split(".").reduce((acc, k) => acc?.[k], json);
...
/**
 * ✅ NOUVEAU — extrait un Article complet d'une réponse API dont la forme
 * peut varier ({data: Article}, {data: {article: Article}}, ou Article
 * brut). Évite le crash silencieux de l'ancienne expression
 * `fullArticle.data ?? fullArticle.data.article ?? fullArticle`, qui
 * levait une TypeError si `fullArticle.data` était undefined (accès à
 * `.article` sur undefined) — empêchant alors silencieusement l'éditeur
 * de recevoir l'article (et donc destinationId) après création rapide.
 */
...
export default function NewArticleModal({
    ...
    useEffect(() => {
        ...
        .then((r) => r.json())
        .then((json) => {
            const list = extractArray<Category>(json, ["", "data", "data.categories"]);
            setCategories(list);
        })
        .catch((err) => {
    ...
    // ── Chargement des destinations existantes (association optionnelle) ───────
    ...
    const handleSubmit = async () => {
        ...
        // ✅ CORRIGÉ — re-fetch l'article complet (avec destinationId,
        // category, author, etc.) après la création rapide, qui ne
        // renvoie qu'un sous-ensemble de champs. Extraction sécurisée
        // via extractArticle() pour éviter un crash si la forme de la
        // réponse varie.
        const fullArticleJson = await fetch(`${API}/admin/articles/${id}`, {
            headers: getAuthHeaders(),
        }).then((r) => r.json());
