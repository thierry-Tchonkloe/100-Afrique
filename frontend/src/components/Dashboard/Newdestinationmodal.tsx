...
// type GeographicLevel = "pays" | "region" | "ville" | "province" | "";
// type FicheStatus = "brouillon" | "publie" | "archive" | "";
...
// function SelectField({ label, value, placeholder, options, error, icon, disabled, onChange }: SelectFieldProps) {
...
//             {error && <p className="mt-1.5 text-xs text-red-500 font-medium flex items-center gap-1"><AlertCircle size={13} />{error}</p>}
//         </div>
//     );
// }
...
//         if (!form.nom.trim()) newErrors.nom = "Le nom officiel est requis.";
//         if (!form.slug.trim()) newErrors.slug = "Le slug est requis.";
//         else if (!/^[a-z0-9-]+$/.test(form.slug))
//             newErrors.slug = "Minuscules, chiffres et tirets uniquement.";
//         if (!form.niveauGeographique) newErrors.niveauGeographique = "Sélectionnez un niveau.";
//         if (!form.statut) newErrors.statut = "Sélectionnez un statut.";
//         setErrors(newErrors);
...
//             const json = await res.json();
//             const id = json?.data?.id ?? json?.data?.destination?.id ?? json?.id;

//             if (!id) {
//                 setApiError("ID de la destination introuvable.");
//                 return;
//             }
...
//         setTimeout(() => {
//             onClose();
//             setForm({ nom: "", slug: "", niveauGeographique: "", statut: "" });
//             setErrors({});
//             setApiError(null);
...
//             >
//                 {/* Top accent bar */}
//                 <div className="h-1 w-full bg-lineair-to-r from-orange-400 via-orange-500 to-amber-400" />
...
//                                     Démarrer la Création d`une Nouvelle Fiche Destination
//                                 </h2>
//                             </div>
...
//                     <div>
//                         <label className="block text-sm font-semibold text-slate-700 mb-1.5">
//                             <span className="inline-flex mr-1.5 align-middle text-orange-500">
//                                 <Globe size={14} />
//                             </span>
//                             Nom Officiel de la Destination
//                             <span className="text-orange-500 ml-0.5">*</span>
//                         </label>
//                         <input
//                             type="text"
...
//                                 ${errors.nom ? "border-red-400" : "border-slate-200 hover:border-slate-300"}`}
//                         />
//                         {errors.nom && (
...
//                                     focus:border-orange-400 focus:ring-2 focus:ring-orange-100
//                                     ${errors.slug ? "border-red-400" : "border-slate-200 hover:border-slate-300"}`}
//                             />
//                         </div>
//                         <p className="mt-1.5 text-xs text-slate-400">
//                             Le slug doit être en minuscules, sans espace ni caractères spéciaux
//                         </p>
//                         {errors.slug && (
//                             <p className="mt-0.5 text-xs text-red-500 font-medium flex items-center gap-1">
...
//                         ]}
//                         onChange={(v) => {
//                             setForm((prev) => ({ ...prev, niveauGeographique: v as GeographicLevel }));
...
//                         error={errors.statut}
//                         options={[
//                             { value: "brouillon", label: "✏️ Brouillon" },
//                             { value: "publie", label: "✅ Publié" },
//                             { value: "archive", label: "🗄️ Archivé" },
//                         ]}
//                         onChange={(v) => {
//                             setForm((prev) => ({ ...prev, statut: v as FicheStatus }));
...
//                             bg-white border-2 border-slate-200 hover:border-slate-300 hover:text-slate-800 
...
//                             active:scale-95 transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
//                     >
//                         {submitting ? (
